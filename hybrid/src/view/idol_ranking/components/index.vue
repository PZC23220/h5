            ...
            <swiper :options="swiperOption" ref="mySwiper" class="banner_container">
                ...
                <swiper-slide id="swiper1">
                    <ul class="top3" v-if="top3NoneToday == false">
                     ...
                     <router-link :to="'/allRanking?type=today&token='+tokens" class="all_ranking" v-if="top3NoneToday == false">すべて表示</router-link>
                    ...
                    <div class="help">
                        ...
                        <div class="rule_content">
                            ...
                            <p v-show="isShow == 7">
                                こんにちは、Groupyです。<br>いつもご利用頂き、誠にありがとうございます。<br><br>Groupyでは、多くのアイドルは自身の努力やファンの支援の下に、注目度をどんどん上げて、新規ファンを増やせます。以下の方法はおすすめです～<br><br>1.自分の公開動画を<b>SNSアカウントにシェア</b>して、ファンをGroupyに招待すること。<br>2.<b>掲示板</b>を活用して投票にてファンに意見を聞いたり、<b>面白いビデオ</b>で盛り上がったりして、ファンから有力な応援をもらい、Groupyの<b>人気ランキングの上位に入ろう</b>。<br>3.Groupyは、<b>10億人超えの中国市場の「欲しい」を喚起</b>し、人気アイドルの動画をオタクやファンの集まり（Weibo,Blibli,Toutiao,WeChat公式アカウント及び各動画配信アプリ等）に導入しようとしています。<br><br>Groupyで、より多くのファンの応援で夢が叶いますように！
                            </p>
                            <h4 @click="tottleFloor(8)"><span></span><em>コインを現金に換金したいが、どうすればいいでしょうか。</em><img :src="tottleImg" alt=""></h4>
                            <p v-show="isShow == 8">
                                こんにちは、Groupyです。<br>いつもご利用頂き、誠にありがとうございます。<br><br><b>Groupyは、アイドルのコインの獲得数によって予め約束した割合で金額決済を行い、決まった日にアイドルの口座に振込みます。</b><br><br>詳しくはアイドル専用のGroupy-idol アプリの決済詳細ページをご覧下さい～<br>その他何か問題ありましたら、お気軽にidol@groupy.vipまでご連絡ください～
                            </p>
                ...
                <swiper-slide id="swiper2">
                    <ul class="top3" v-if="top3None == false">
                     ...
                     <router-link :to="'/allRanking?type=all&token='+tokens" class="all_ranking">すべて表示</router-link>
